...
@php
    $inputId = $id ?? $name;
    $canCrud = $allowCrud && filled($parameterId);
    $optionList = collect($options)->map(function ($option) {
        ...
        return ['id' => null, 'label' => (string) $option, 'value' => (string) $option];
    })->filter(fn ($o) => $o['value'] !== '')->values()->all();

    $apiBase = $canCrud ? url('/settings/parameters/'.$parameterId.'/details') : null;
@endphp

<div
    x-data="parameterAutocomplete({
        prop: @js($name),
                ...
                <template x-for="(option, index) in filtered" :key="(option.id || option.value) + '-' + index">
                    <div class="px-2 py-1">
                        <div x-show="editingId !== option.id"
                            ...
                            <span class="flex items-center justify-center w-8 h-8 rounded-md bg-gray-100 text-gray-500 shrink-0">
